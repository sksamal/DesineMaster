...
Parameters::Parameters(const string &parameter_file)
{
    ...
    f_random   	= (RandomVar*) 0;
    f_buffer   	= (RandomVar*) 0;
    f_duration1 = (RandomVar*) 0;
    f_duration2 = (RandomVar*) 0;
    f_bandwidth = (RandomVar*) 0;
    l_capacity  = (RandomVar*) 0;
    hl_capacity  = (RandomVar*) 0;
...
{
    TRACE("Parameters::~Parameters -->");
    if (f_random) delete f_random;
    if (f_arrival) delete f_arrival;
    if (f_duration1) delete f_duration1;
    if (f_duration2) delete f_duration2;
    if (f_bandwidth) delete f_bandwidth;
    if (l_capacity) delete l_capacity;
    if (hl_capacity) delete hl_capacity;
...
void Parameters::print(ostream &buffer) const
{
   ...
   // topology.print(buffer);
    ...
    << Tag::F_RANDOM << "  ";
    f_random->print(buffer);
    buffer << endl

   //added for obs by suraj y.
    << Tag::F_BUFFER << " ";
    f_buffer->print(buffer);
    buffer << endl

    << Tag::F_DURATION1 << "  ";
    f_duration1->print(buffer);
    buffer << endl

    << Tag::F_DURATION2 << "  ";
    f_duration2->print(buffer);
    buffer << endl
...
void Parameters::readParameters(const string &parameter_file)
{
    ...
    ts = preader.getParameter(Tag::BUFF_TYPE);
    if (!ts.empty())
    {
    	Buff_Type = atoi(ts.front().c_str());
    }

    ts = preader.getParameter(Tag::SIM_TYPE);
       if (!ts.empty())
       {
    	   Sim_Type = atoi(ts.front().c_str());
       }
    ...
    ts = preader.getParameter(Tag::HL_CAPACITY);
    if (!ts.empty())
    {
        hl_capacity = createDistribution(ts);
    }
    ...
    ts = preader.getParameter(Tag::F_RANDOM);
       if (!ts.empty())
       {
    	   f_random = createDistribution(ts);
       }

    ts = preader.getParameter(Tag::F_DURATION1);
    if (!ts.empty())
    {
        f_duration1 = createDistribution(ts);
    }
    ts = preader.getParameter(Tag::F_DURATION2);
        if (!ts.empty())
        {
            f_duration2 = createDistribution(ts);
        }
    ...
    //added for obs by suraj y.
    ts = preader.getParameter(Tag::F_BUFFER);
        if (!ts.empty())
        {
            f_buffer = createDistribution(ts);
        }
